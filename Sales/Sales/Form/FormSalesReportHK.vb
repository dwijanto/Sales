...
Public Class FormSalesReportHK
    ...
    Sub DoWork()
        ...
        Filename = SelectedPath & "\" & String.Format("HK Sales {0} {1}-{2}.xlsx", "Report", Format(startdate, "ddMMMyyyy"), Format(enddate, "ddMMMyyyy"))
        ...
        Try
            ...
            If PublicClass.myLocation = MyLocation.HongKong Then
                If HelperClass1.UserInfo.IsSalesTeamHK Then
                   ...
                   " (select invid,invdate,date_part('Year',invdate) as year,date_part('month',invdate) as month,date_part('day',invdate) as day,orderno,tx.customerid,c.customername,reportcode,saleforce,country,custtype,salesman,shipto,substring(productid,1,6) as refno,productid,tx.cmmf,sbu,family.productfamily,brand,cmmf.materialdesc,supplierid,null::integer,null::numeric(13,2),null::numeric(15,5),qty,totalsales ,totalcost,qty as totalqty ,totalsales as totalsales,totalcost as totalcost,region,location,cm.bpcode,cm.mlacode,sales.get_producttypebrand(sbu,brand) as producttypebrand" &
                   " from sales.tx " &
                ...
                Else
                    ...
                    obj.strsql = "with cmmf as (" &
                                " select distinct cmmf,first_value(materialdesc) over (partition by cmmf order by invdate desc,cmmf,materialdesc  )as materialdesc  from sales.tx) " &
                                ", family as (select distinct cmmf,first_value(productfamily) over (partition by cmmf order by invdate desc,cmmf,productfamily  )as productfamily from sales.tx ) " &
                        "(select invid,invdate,date_part('Year',invdate) as year,date_part('month',invdate) as month,date_part('day',invdate) as day,orderno,tx.customerid,c.customername,reportcode,saleforce,country,custtype,salesman,shipto,substring(productid,1,6) as refno,productid,cmmf.cmmf,sbu,family.productfamily,brand,cmmf.materialdesc,supplierid,qty,totalsales,totalcost ,qty as totalqty ,totalsales as totalsales,totalcost as totalcost,region,location,cm.bpcode,cm.mlacode,sales.get_producttypebrand(sbu,brand) as producttypebrand" &
                         " from sales.tx " &
                        " left join cmmf on cmmf.cmmf = tx.cmmf " &
                         " left join family on family.cmmf = tx.cmmf" &
                         " left join sales.customer c on c.customerid = tx.customerid " &
    ...
    Private Sub CreatePivotTable(ByVal oxl As Excel.Application, ByVal oWb As Excel.Workbook, ByVal isheet As Integer, ByVal mydate As Date)
        ...
        End With
        osheet.PivotTables("PivotTable1").CalculatedFields.Add("GP", "=iferror((totalsales-totalcost )/totalsales,0)", True)
        ...
        osheet = oWb.Worksheets(isheet)
        'oWb.PivotCaches.Create(Excel.XlPivotTableSourceType.xlDatabase, "DATA!ExternalData_1").CreatePivotTable(osheet.Name & "!R6C1", "PivotTable1", Excel.XlPivotTableVersionList.xlPivotTableVersionCurrent)
        oWb.Worksheets("Pivot Table").PivotTables("PivotTable1").PivotCache.CreatePivotTable(osheet.Name & "!R6C1", "PivotTable1", Excel.XlPivotTableVersionList.xlPivotTableVersionCurrent)
        With (osheet.PivotTables("PivotTable1"))
            .ingriddropzones = True
            .RowAxisLayout(Excel.XlLayoutRowType.xlTabularRow)           
        End With
        ...
        osheet.PivotTables("PivotTable1").PivotFields("producttypebrand").Orientation = Excel.XlPivotFieldOrientation.xlColumnField
        ...
        osheet.PivotTables("PivotTable1").PivotFields(" Total sales").NumberFormat = "#,##0.00"
        osheet.Cells.EntireColumn.AutoFit()
        ...
        osheet.PivotTables("PivotTable1").PivotFields("customerid").Subtotals = {False, False, False, False, False, False, False, False, False, False, False, False}        
        osheet.PivotTables("PivotTable1").AddDataField(osheet.PivotTables("PivotTable1").PivotFields("GP"), " GP%", Excel.XlConsolidationFunction.xlSum)
        osheet.PivotTables("PivotTable1").AddDataField(osheet.PivotTables("PivotTable1").PivotFields("totalcost"), " Total Cost", Excel.XlConsolidationFunction.xlSum)
        osheet.PivotTables("PivotTable1").AddDataField(osheet.PivotTables("PivotTable1").PivotFields("GP"), " GP%ofColumn", Excel.XlConsolidationFunction.xlSum)

        With osheet.PivotTables("PivotTable1").PivotFields(" GP%ofColumn")
            .Calculation = Excel.XlPivotFieldCalculation.xlPercentOfColumn
            .NumberFormat = "0.00%"
        End With
        osheet.PivotTables("PivotTable1").PivotFields(" Total Cost").NumberFormat = "#,##0.00"
        osheet.PivotTables("PivotTable1").PivotFields(" GP%").NumberFormat = "0.00%"
        osheet.Cells.EntireColumn.AutoFit()
    End Sub
