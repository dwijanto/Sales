...
Public Class FormCustomerBUDateRange
        ...
        Try
            ...
            If PublicClass.myLocation = MyLocation.HongKong Then
                If HelperClass1.UserInfo.IsSalesTeamHK Then
                    obj.strsql = "(select invid,invdate,orderno,tx.customerid,c.customername,reportcode,saleforce,country,custtype,salesman,shipto,productid,cmmf,sbu,productfamily,brand,materialdesc,supplierid,qty ,totalsales, totalcost,region,location,invdate as filterdate1,invdate as filterdate2" &
                       " from sales.tx " &
                       " left join sales.customer c on c.customerid = tx.customerid " &
                       " where invdate >= " & DateFormatyyyyMMdd(CDate(startdate)) & " and invdate <= " & DateFormatyyyyMMdd(CDate(enddate)) & mycriteria & mybuCriteria & " order by invdate);"
                Else
                    obj.strsql = "(select invid,invdate,orderno,tx.customerid,c.customername,reportcode,saleforce,country,custtype,salesman,shipto,productid,cmmf,sbu,productfamily,brand,materialdesc,supplierid,qty ,totalsales,null::numeric(15,5) as  totalcost,region,location,invdate as filterdate1,invdate as filterdate2" &
                       " from sales.tx " &
                       " left join sales.customer c on c.customerid = tx.customerid " &
                       " where invdate >= " & DateFormatyyyyMMdd(CDate(startdate)) & " and invdate <= " & DateFormatyyyyMMdd(CDate(enddate)) & mycriteria & mybuCriteria & " order by invdate);"
                End If
            ...
            '" where invdate >= " & DateFormatyyyyMMdd(CDate(startdate)) & " and invdate <= " & DateFormatyyyyMMdd(CDate(enddate)) & mycriteria & " order by invdate); "
    ...
    Private Sub CreatePivotTable(ByVal oxl As Excel.Application, ByVal oWb As Excel.Workbook, ByVal isheet As Integer, ByVal mydate As Date)
        ...
        osheet.PivotTables("PivotTable1").AddDataField(osheet.PivotTables("PivotTable1").PivotFields("qty"), " Qty ", Excel.XlConsolidationFunction.xlSum)
