        ...
        Try
            ...
            If PublicClass.myLocation = MyLocation.HongKong Then
                ...
                If HelperClass1.UserInfo.IsSalesTeamHK Then
                    ...
                    '    ",rp.retailprice,case sbu when 'COOKWARE' then  null else (1 - totalsales / rp.retailprice)  end as sda,case brand when 'LAGOSTINA' then (1 - totalsales / rp.retailprice)  when ('LAGOSTINA CASA')  then (1 - totalsales / rp.retailprice)  end as lagocookware ,case sbu when 'COOKWARE' then( case brand when 'TEFAL' then (1 - (totalsales / (rp.retailprice * 0.7))) end )end as tefalcookwaretradediscount,case sbu when 'COOKWARE' then( case brand when 'TEFAL' then (1 - (totalsales / (rp.retailprice ))) end )end as tefalcookwaredirectdiscount" &
                    '    " from sales.tx " &
                    '    " left join sales.customer c on c.customerid = tx.customerid " &
                    '    " left join cmmf on cmmf.cmmf = tx.cmmf " &
                    '    " left join sales.hkretailprice rp on rp.cmmf = tx.cmmf and year = " & startdate.Year - 1 &
                    '    " where invdate >= " & DateFormatyyyyMMdd(CDate(startdate.Year - 1 & "-" & startdate.Month & "-" & startdate.Day)) & " and invdate <= " & DateFormatyyyyMMdd(CDate(enddate.Year - 1 & "-12-31")) & mycriteria & " order by invdate) union all " &
                    ...
                    '     " left join sales.customer c on c.customerid = tx.customerid " &
                    '     " left join cmmf on cmmf.cmmf = tx.cmmf " &
                    '      " left join sales.hkretailprice rp on rp.cmmf = tx.cmmf and year = " & startdate.Year &
                    '    " where invdate >= " & DateFormatyyyyMMdd(startdate) & " and invdate <= " & DateFormatyyyyMMdd(enddate) & mycriteria & " order by invdate)"
                    obj.strsql = "with cmmf as (" &
                       " select distinct cmmf,first_value(materialdesc) over (partition by cmmf order by invdate desc,cmmf,materialdesc  )as materialdesc  from sales.tx) " &
            ...
            Else
                ...
                '    " where invdate >= " & DateFormatyyyyMMdd(CDate(startdate.Year - 1 & "-" & startdate.Month & "-" & startdate.Day)) & " and invdate <= " & DateFormatyyyyMMdd(CDate(enddate.Year - 1 & "-12-31")) & mycriteria & " order by invdate) union all " &
                '    "(select invid,invdate,orderno,tx.customerid,c.customername,reportcode,saleforce,country,custtype,salesman,shipto,productid,cmmf,sbu,productfamily,brand,materialdesc,supplierid,null::integer,null::numeric(13,2),null::numeric(15,5),qty,totalsales ,totalcost,qty as totalqty ,totalsales as totalsales,totalcost as totalcost,region,location," &
                '    " date_part('month',invdate) as month,sales.retur as salesreturn,sales.get_sales(retur,totalsales) as sales,sales.get_return(retur,totalsales) as return,sales.get_salesreversal(retur,totalsales) as salesreversal," &
                '    " custtype as channel,custname,f.type as ec,f.id as famlv1,merch,storename,mlacode,m.mlaname ,posid,od,invdate as filterdate1,invdate as filterdate2" &
                '    " from sales.txtw tx " &
                '    " left join sales.customer c on c.customerid = tx.customerid " &
                '    " left join sales.tw_family f on f.familyname = tx.productfamily" &
            ...
            Select Case id
                ...
                Case 6
                    If PublicClass.myLocation = MyLocation.HongKong Then
                        ...
                        CheckedListBox1.ValueMember = "customerid"
                    Else
                        CheckedListBox1.DataSource = customerbs
                        CheckedListBox1.DisplayMember = "customername"
